...
class CompatibleProductTest < ActiveSupport::TestCase
  ...
  class ForProductWithTagsTests < ActiveSupport::TestCase
    setup do
    ...
    test 'seat should be compatible with stroller via adapter_a and no tags' do
      assert_equal [CompatibleProduct.new(product: @stroller, adapter: @adapter_a)],
                   CompatibleProduct.for_product(@seat)
    end

    test 'stroller should be compatible with seat via adapter_a and with tag_a' do
      assert_equal [CompatibleProduct.new(product: @seat, adapter: @adapter_a, tags_from_link: [@tag_a])],
                   CompatibleProduct.for_product(@stroller)
    end

    # then:
    test 'adapter_b should not be compatible with stroller, but not with seat' do
      assert_equal [CompatibleProduct.new(product: @seat, adapter: @adapter_b, tags_from_link: [@tag_a])],
                   CompatibleProduct.for_product(@adapter_b)
    end

    test 'adapter_a should be compatible with stroller and seat' do
      assert_equal [
